...
import Controllers.MangReportsDataContoller;
...
public class Restaurant implements Serializable{
	...
	private HashSet<DeliveryPerson> onMission;
	private HashMap<DeliveryPerson,Integer>delsByDp;

	
	public HashMap<DeliveryPerson, Integer> getDelsByDp() {
		return delsByDp;
	}

	public void setDelsByDp(HashMap<DeliveryPerson, Integer> delsByDp) {
		this.delsByDp = delsByDp;
	}
	...
	private Restaurant() {
		...
		onMission = new HashSet<DeliveryPerson>();
		delsByDp = new HashMap<DeliveryPerson,Integer>();
	...
	public boolean addOrder(Order order) {
		try {
			...
			fillOrdersDataStructures(order);
	...
	public boolean addDelivery(Delivery delivery) {
		...
		Integer amount = delsByDp.get(delivery.getDeliveryPerson());
		if(amount == null) {
			amount = 1;
		}
		else
			amount++;
		delsByDp.put(delivery.getDeliveryPerson(), amount);
	...
	public boolean removeOrder(Order order) {
		if(order == null || !getOrders().containsKey(order.getId()))
			return false;
		this.orderByCustomer.get(order.getCustomer()).remove(order);
		for(DeliveryArea da: this.orderByDeliveryArea.keySet()) {
			for(Order o: this.orderByDeliveryArea.get(da)) {
				if(o.equals(order)) {
					this.orderByDeliveryArea.get(da).remove(order);
					break;
				}
			}
		}
		if(getOrders().remove(order.getId())!=null) {
			if( order.getDelivery() != null && order.getDelivery() instanceof RegularDelivery) {
				RegularDelivery rd = (RegularDelivery) order.getDelivery();
				if(rd != null) {
				 rd.removeOrder(order);
				}
				 return true;
			}
	...
	public boolean removeDelivery(Delivery delivery) {
		...
		onMission.remove(delivery.getDeliveryPerson());
		Integer amount = delsByDp.get(delivery.getDeliveryPerson());
		if(amount > 0) {
			amount--;
		}
	...
	public boolean removeDeliveryArea(DeliveryArea oldArea, DeliveryArea newArea) {
		...
		orderByDeliveryArea.remove(oldArea);
	...
	public void deliver(Delivery d) {
		...
		if(d instanceof RegularDelivery) {
			RegularDelivery rd = (RegularDelivery)d;
			for(Order o : rd.getOrders()) {
				s += "Order " + o.getId() + " had reached to Customer " + o.getCustomer().getFirstName() + " " + o.getCustomer().getLastName() + "\n";
			}
		}
		else {
			ExpressDelivery ed = (ExpressDelivery)d;
			s = "Order " + ed.getOrder().getId()+" had reached to Customer " + ed.getOrder().getCustomer().getFirstName() + " " + ed.getOrder().getCustomer().getLastName() + "\n";
		}
	...
	public double revenueFromExpressDeliveries() {
		...
		HashMap<Customer, Double> revenueByCust = new HashMap<Customer, Double>();
		for(Delivery d: getDeliveries().values()) {
			if(d instanceof ExpressDelivery) {
				ExpressDelivery ed = (ExpressDelivery)d;
				if(ed.getOrder()!= null) {
					amountOfPostages+=ed.getPostage() + ed.getOrder().calcOrderRevenue();;
					customers.add(ed.getOrder().getCustomer());
					Double income = revenueByCust.get(ed.getOrder().getCustomer());
					if(income == null) {
						income = 30.0;
					}
					income += ed.getPostage() + ed.getOrder().calcOrderRevenue();
					revenueByCust.put(ed.getOrder().getCustomer(), income);
				}
			}
		}
		MangReportsDataContoller.revenuByCust = revenueByCust;
		amountOfPostages+=(30*customers.size());
		return amountOfPostages;
	}
	
	public LinkedList<Component> getPopularComponents(){
		final HashMap<Component, Integer> componentsandAmount = new HashMap<>();
		for(Dish d: getDishes().values()) {
			for(Component c: d.getComponenets()) {
		...
		MangReportsDataContoller.componentsandAmounts = componentsandAmount;
	...
	public TreeSet<Dish> getProfitRelation(){
		...
		for(Dish d: getDishes().values()) {
			profit.add(d);
			MangReportsDataContoller.relation.put(d, d.getPrice()/d.getTimeToMake());
		}
		return profit;
	}
	...
	public TreeSet<Delivery> createAIMacine(DeliveryPerson dp, DeliveryArea da, TreeSet<Order> orders){
		...
		int IDCounter = aM.getBiggestID(MainController.getRest().getDeliveries().keySet()) + 1;
	...
	public void  serialize() {
		...
		try {
			//FileOutputStream fileOut = new FileOutputStream("Rest.ser");//name of the folder we create
	...
	public static Restaurant deserialize() {
		...
		try {
	    	 ...
	    	 JOptionPane.showMessageDialog(null, "Rest.ser does not exist!");
	      } catch (ClassNotFoundException c) {
	    	  JOptionPane.showMessageDialog(null, "Restaurant class was not found!");
	         c.printStackTrace();
	      }
		return r;     
	}
 
	//fill data structures:(order by customer and order by area)
	private void fillOrdersDataStructures(Order order){
		
		TreeSet<Order> o;
		o = orderByCustomer.get(order.getCustomer());
		if(o == null) {
			o = new TreeSet<Order>(); 
		}
		o.add(order);
		orderByCustomer.put(order.getCustomer(), o);
		HashSet<Order> oByArea;
			for(DeliveryArea area: this.areas.values()) {
				if(area.getNeighberhoods().contains(order.getCustomer().getNeighberhood())) {
					oByArea = orderByDeliveryArea.get(area);
					if(oByArea == null) {
						oByArea = new HashSet<Order>();
					}
					oByArea.add(order);
					orderByDeliveryArea.put(area, oByArea);
					break;
				}
			}
		}
	...
	//pick a delivery person for the deliveries it takes someon who doesnt have any deliveries
	//to deliver if there no one it picks the first delivey person that belongs to the area
	private DeliveryPerson pickDelPerson(DeliveryArea a) {
		DeliveryPerson toReturn = null;
		for(DeliveryPerson dP: a.getDelPersons()) {
			if( onMission != null && !onMission.contains(dP)) {
				toReturn = dP;
				break;
			}
		}
		if(toReturn == null) {
			for(DeliveryPerson dP : this.getDeliveryPersons().values()) {
				if(dP.getArea().equals(a)) {
					toReturn = dP;
					break;
				}
			}
	...
	//create new deliveries by using AI Machine(called  by the pack orders button)
	public void CreateDeliveries() {
		
		TreeSet<Order> ordersTOAi;
		if(onMission == null) {
			onMission = new HashSet<DeliveryPerson>();
		}
		for(DeliveryArea area: orderByDeliveryArea.keySet()) {
			ordersTOAi = new TreeSet<Order>();
			for(Order o: orderByDeliveryArea.get(area)) {
				if(o.getDelivery() == null) {
					ordersTOAi.add(o);
				}
			}
			DeliveryPerson dp = pickDelPerson(area);
			this.onMission.add(dp);
			if(ordersTOAi != null && !ordersTOAi.isEmpty()) {
				TreeSet<Delivery> toAdd = this.createAIMacine(dp, area, ordersTOAi);
				for(Delivery d: toAdd) {
					this.addDelivery(d);
				}
			}
		}
